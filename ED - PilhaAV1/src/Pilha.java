public class Pilha {
        int topo;
        int vetor_elementos[];

        public Pilha(int tamanho) {
            this.topo = 0;
            this.vetor_elementos = new int[tamanho];
        }

        public int Quantidade() {
            return this.topo;
        }

        public boolean Pilha_Vazia() {
            if (topo == 0) {
                return true;
            } else {
                return false;
            }
        }

        public boolean Pilha_Cheia() {
            if (topo >= vetor_elementos.length) {
                return true;
            } else {
                return false;
            }
        }

        public void Empilhar_Pilha(int valor) {
            if (Pilha_Cheia()) {
                System.out.println("Pilha Cheia");
            } else {
                vetor_elementos[topo] = valor;
                topo += 1;

            }
        }

        public void Desempilhar_Pilha() {
            if (Pilha_Vazia()) {
                System.out.println("Pilha Vazia");
            } else {
                int elemento_removido = vetor_elementos[topo-1];
                topo -= 1;

            }
        }
        public void Ordenar_Pilha() {
            if (Pilha_Vazia()) {
                System.out.println("Pilha vazia");
            } else {
                for (int i = topo; i < vetor_elementos.length; i++) {
                    for (int j = topo+1; j < vetor_elementos.length; j++) {
                        if (vetor_elementos[topo] > vetor_elementos[j]) {
                            int var_aux = vetor_elementos[i];
                            vetor_elementos[i] = vetor_elementos[j];
                            vetor_elementos[j] = var_aux;
                        }
                    }
                }
            }
        }
        public int Ver_Topo()
        {
            return vetor_elementos[topo-1];
        }

        public void eliminarMaiorMenor(){
            System.out.println("Visao da Pilha");
            Pilha pilhAux = new Pilha(topo);
            int maior = Ver_Topo();
            int menor = Ver_Topo();
            int salvarTopo = Ver_Topo();
            while (!Pilha_Vazia()){
                System.out.println(Ver_Topo());
                salvarTopo = Ver_Topo();
                if(maior < Ver_Topo()){
                    maior = Ver_Topo();
                }
                if(menor > Ver_Topo()){
                    menor = Ver_Topo();
                }
                Desempilhar_Pilha();
                pilhAux.Empilhar_Pilha(salvarTopo);
            }
            System.out.println("Pilha Invertida");
            while (!pilhAux.Pilha_Vazia()){
                if((pilhAux.Ver_Topo() == maior) || (pilhAux.Ver_Topo()) == menor ){
                    pilhAux.Desempilhar_Pilha();
                }
                else {
                    salvarTopo = pilhAux.Ver_Topo();
                    pilhAux.Desempilhar_Pilha();
                    Empilhar_Pilha(salvarTopo);
                }
                if(!Pilha_Vazia()) {
                    System.out.println(Ver_Topo());
                }
            }
        }

        public Pilha ordenarPilha(){
                Pilha pilhaAux = new Pilha(topo);
                int salvarTopo;
            System.out.println("Pilha Ordenada");
                while(!Pilha_Vazia())
                {
                    salvarTopo = Ver_Topo();
                    Desempilhar_Pilha();
                    while(!pilhaAux.Pilha_Vazia() && (pilhaAux.Ver_Topo()>salvarTopo))
                    {
                        Empilhar_Pilha(pilhaAux.topo);
                        pilhaAux.Desempilhar_Pilha();
                    }
                    pilhaAux.Empilhar_Pilha(salvarTopo);
                }
                return pilhaAux;
            }
        public void verPilha(){
            Pilha pilhAux = new Pilha(topo);
            int salvarTopo = Ver_Topo();
            while (!Pilha_Vazia()){
                System.out.println(Ver_Topo());
                salvarTopo = Ver_Topo();
                Desempilhar_Pilha();
                pilhAux.Empilhar_Pilha(salvarTopo);
            }
            while (!pilhAux.Pilha_Vazia()){
                salvarTopo = pilhAux.Ver_Topo();
                pilhAux.Desempilhar_Pilha();
                Empilhar_Pilha(salvarTopo);
            }
        }
        }


